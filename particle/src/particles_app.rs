...
impl App for ParticleApp {
    ...
    fn update(&mut self, delta_time: f32, context: &Context) {
        ...
        self.particles.iter_mut().for_each(|p : &mut Particle | {
            p.position[0] = p.position[0] + delta_time* p.velocity[0] + rng.gen_range(-0.5..0.5)/150.0;
            p.position[1] = p.position[1] + delta_time* p.velocity[1] + rng.gen_range(-0.5..0.5)/150.0;
            p.position[2] = p.position[2] + delta_time* p.velocity[2] + rng.gen_range(-0.5..0.5)/150.0;
        });
